...
//   dogs/{slug}/meta.json     -> { name, owner, startDate, endDate, createdAt }
...
export async function createDog({ name, owner, startDate, endDate }) {
...
export async function updateDogDates(slug, startDate, endDate) {
  const meta = await getDogMeta(slug);
  if (!meta) return null;
  meta.startDate = startDate || "";
  meta.endDate = endDate || "";

  await put(`dogs/${slug}/meta.json`, JSON.stringify(meta), {
    access: "public",
    contentType: "application/json",
    addRandomSuffix: false,
    allowOverwrite: true,
